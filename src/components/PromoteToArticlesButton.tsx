...
export function PromoteToArticlesButton() {
  ...
  async function handlePromote() {
    ...
    setError(null)
    try {
      const res = await fetch('/api/promote-test-article', {
        method: 'POST',
  ...
  if (done) return <div style={{ padding: '8px 16px', background: '#22c55e', color: 'white', borderRadius: '6px', fontWeight: 'bold' }}>✅ Promoted to Articles!</div>
  ...
  return (
    <div style={{ marginBottom: '12px' }}>
      ...
      >
        {loading ? 'Promoting...' : '🚀 Promote to Articles'}
      </button>
